...
import fr.keyser.nn.fsm.impl.ActionConfiguration;
import fr.keyser.nn.fsm.impl.ActionFunction;
import fr.keyser.nn.fsm.impl.ActionListenerSource;
import fr.keyser.nn.fsm.impl.AutoListener;
import fr.keyser.nn.fsm.impl.AutoTransitionSource;
import fr.keyser.nn.fsm.impl.Automats;
import fr.keyser.nn.fsm.impl.AutomatsListener;
import fr.keyser.nn.fsm.impl.ChoiceListener;
import fr.keyser.nn.fsm.impl.ChoiceListener.StateChoiceEssence;
import fr.keyser.nn.fsm.impl.ChoicePredicate;
import fr.keyser.nn.fsm.impl.CreateChildFunction;
import fr.keyser.nn.fsm.impl.DirectTransitionSource;
import fr.keyser.nn.fsm.impl.Graph;
import fr.keyser.nn.fsm.impl.Graph.GraphBuilder;
import fr.keyser.nn.fsm.impl.JoinTransitionSource;
import fr.keyser.nn.fsm.impl.MergeFunction;
import fr.keyser.nn.fsm.impl.MergingTransitionSource;
import fr.keyser.nn.fsm.impl.NodeType;
import fr.keyser.nn.fsm.impl.RegionConfiguration;
import fr.keyser.nn.fsm.impl.RegionListener;
import fr.keyser.nn.fsm.impl.TerminalListener;
import fr.keyser.nn.fsm.impl.TransitionGuard;
import fr.keyser.nn.fsm.impl.TransitionGuardListener;
...
public class AutomatsBuilder<T> implements Container<T> {
	...
	protected void register(GraphBuilder assembler) {
	    assembler.node(state(), getType())
	            .transition(essence.asTransitionSource());
	}
	...
	protected void register(GraphBuilder assembler) {
	    ...
	    if (initial != null)
		assembler.node(state(), getType())
		        .first(assembler.node(initial.state(), initial.getType()));

	    if (mergingAllowed)
		assembler.node(state(), getType())
		        .transition(MergingTransitionSource.INSTANCE);
	...
	protected void register(GraphBuilder assembler) {
	    ...
	    if (!to.isEmpty()) {
		assembler.node(state(), getType())
		        .transition(new DirectTransitionSource(to));
	    }
	...
	RegionConfiguration<T> asConfiguration() {
	    ...
	    Set<State> childs = getEdges().keySet().stream()
	            .filter(s -> !s.equals(joinState))
	...
	protected void register(GraphBuilder assembler) {
	    assembler.node(state(), getType())
	            .transition(new AutoTransitionSource(findFirstChildState(to.state())));
	}
	...
	return new Automats<>(createGraph(), assemble().peekLast(), null);
    }
	...
	return new RegionListener<>(next, regions.stream()
	        .map(InnerRegion::asConfiguration)
	        .collect(Collectors.toList()));
    }
