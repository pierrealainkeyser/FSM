...
import fr.keyser.nn.fsm.impl.Transition;
...
public class SpringELChoice<T> implements ChoicePredicate<T> {
	...
	Optional<T> parentValue = Optional.ofNullable(instance.getParent())
	        .flatMap(i -> i.opt(Function.identity()));

	return value.map(v -> testValue(v, parentValue, t))
	        .orElse(false);
